...
class CustomerPricingPortal(http.Controller):
    ...
    def portal_my_pricing(self, **kw):
        try:
            ...
            # Get success/error parameters for template
            success_param = request.httprequest.args.get('success')
            error_param = request.httprequest.args.get('error')
            ...
            # Use with_user instead of sudo for proper access control
            quotations = request.env['sale.order'].with_user(user).search([
    ...
    def confirm_order(self, order_id, **kw):
        try:
            ...
            # Check access rights properly
            if not order.check_access_rights('write', raise_exception=False):
                return request.redirect('/my/pricing?error=access')

            if order.state in ['draft', 'sent']:
                # Use sudo only for the confirmation action
    ...
    def cancel_order(self, order_id, **kw):
        try:
            ...
            if order.state in ['draft', 'sent']:
                # Use sudo only for the cancellation action
